...
def load_data(data_dir, test_subjects, gestures, window_size):
    ...
    for recording in recordings:
        dfs[gestures.index(recording["gesture"])].append(
            pd.read_csv(recording["raw_data_filepath"], header=None)
        )
        if USE_FFT:
            fft_dfs[gestures.index(recording["gesture"])].append(
                pd.read_csv(recording["fft_data_filepath"], header=None)
            )
    ...
    for i in range(len(gestures)):
        concat_dfs.append(
            pd.concat(dfs[i], ignore_index=True).iloc[
                :, 0 : (NUM_SENSORS * NUM_READINGS)
            ]
        )
        if USE_FFT:
            concat_fft_dfs.append(
                pd.concat(fft_dfs[i], ignore_index=True).iloc[
                    :, 0 : (NUM_SENSORS * NUM_FFT_READINGS)
                ]
            )

    if USE_FFT:
        for i in range(len(concat_fft_dfs)):
            concat_fft_dfs[i] = concat_fft_dfs[i].iloc[
                :,
                [
                    i * NUM_FFT_READINGS + j + 1
                    for i in range(0, NUM_SENSORS)
                    for j in range(NUM_FFT_READINGS - 1)
                ],
            ]
            concat_fft_dfs[i].columns = range(concat_fft_dfs[i].columns.size)

    preprocessor = EMG_preprocessor()

    windows = [[] for _ in range(len(gestures))]
    data_arrays = [[] for _ in range(len(gestures))]
    for i, df in enumerate(concat_dfs):
        data_arrays[i], windows[i] = preprocess_per_sensor_avg(
            df,
            preprocessor,
            i,
            num_sensors=NUM_SENSORS,
            window_size=window_size,
            window_increment=(window_size // 5) + (window_size // 5 % 2),
        )
    ...
    for i in range(len(gestures)):
        features[i] = np.hstack(
            (
                get_features_per_sensor(windows[i], feature_groups=("HJORTH", "HTD")),
                np.full((windows[i].shape[0], 1), i),
            )
        )
    ...
    if USE_FFT:
        ...
        for i, df in enumerate(concat_fft_dfs):
            fft_data_arrays[i], fft_windows[i] = preprocess_per_sensor_fft(
                df,
                i,
                num_sensors=NUM_SENSORS,
                window_size=window_size,
                window_increment=(window_size // 5) + (window_size // 5 % 2),
                num_readings=NUM_FFT_READINGS,
            )

        fft_features = [[] for _ in range(len(gestures))]
        for i in range(len(gestures)):
            fft_features[i] = np.hstack(
                (
                    get_fft_features_per_sensor(fft_windows[i]),
                    np.full((fft_windows[i].shape[0], 1), i),
                )
            )
...
def preprocess_per_sensor_avg(
    ...
    for sensor in range(num_sensors):
        df[f"avg_{sensor}"] = df.iloc[
            :, sensor * num_readings : sensor * num_readings + num_readings
        ].mean(axis=1)
    raw_signal = df.iloc[:, -NUM_SENSORS:].values
    raw_signal = get_windows(
        raw_signal, window_size=window_size, window_increment=window_increment
    )
...
def preprocess_per_sensor_fft(
    ...
    raw_signal = get_windows(
        raw_signal_wrapped, window_size=window_size, window_increment=window_increment
    )
...
def preprocess_per_sensor(
    df, preprocessor, class_name, num_sensors=4, window_size=200, window_increment=100
):
    raw_signal = df.values
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="gilbert",
        help="Subject to use for classification",
    )
    parser.add_argument(
        "-g",
        "--gestures",
        nargs="+",
        default=("fist", "index", "ok", "peace", "thumb", "up", "down"),
    ...
    parser.add_argument(
        ...
        type=int,
        default=50,
        help="Number of readings before making a new prediction",
    )
    ...
    X_feat, y_feat = load_data(
        args.data_dir, [args.subject], args.gestures, args.window_size
    )
