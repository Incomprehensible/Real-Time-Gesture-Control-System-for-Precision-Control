...
def plot_cycle_lines():
    ...
    for d in range(max_devices):
        ...
        for x in range(plot_len):
            xy.append(
                [DX + x * x_scale, DY + (plot_emg[d][x] - y_zero[d]) * y_scale[d]]
            )
        cl = num_to_color(d)
...
def plot_cycle_spg():
    ...
    for d in range(max_devices):
        ...
        DY = (
            height / 2 - YS * 6 * active_devices / 2 + YS * 6 * (cur_devices - 1)
        )  # (1+active_devices) * (d+1)
...
def plot_cycle_tester():
    ...
    for d in range(max_devices):
        ...
        for x in range(plot_len):
            ...
            xy.append(
                [DX + x * x_scale, DY + (plot_emg[d][x] - 0) * height * 0.5 / 32768]
            )
        cl = num_to_color(d)
        pygame.draw.lines(screen, cl, False, xy)
        ...
        if batt_fh < 2:
            batt_fh = 2
        screen.fill(
            cl, (batt_dx + 1, batt_dy + batt_h - batt_fh - 1, batt_w - 2, batt_fh)
        )
